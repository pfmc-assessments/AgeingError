...
A function to generate simulated double reading data with given
properties
...
- SelexForm:
  ...
  Form of selectivity-at-age (logistic selex-at-age is the only one that
  is implemented).
...
- ReadsMat:
  ...
  Matrix describing number of reads per reader combination. Where each
  row specifies how many reads (in the first column) have a particular
  pattern of double reads (in the second through Nreaders+1 columns).

- RecCv:

  CV of recruitment, and it shoudl be noted that recruitment is assumed
