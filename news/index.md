...
- Deprecated two obsolete functions to reduce confusion
  ...
  - [`PlotOutputFn()`](http://pfmc-assessments.github.io/AgeingError/reference/PlotOutputFn.md)
    replaced by
    [`plot_output()`](http://pfmc-assessments.github.io/AgeingError/reference/plot_output.md)

## AgeingError 2.1.0

- Renamed some functions to have more accurate names and match the
...
- Added new functions to help with creating the data and specs files
  ...
  - [`write_files()`](http://pfmc-assessments.github.io/AgeingError/reference/write_files.md):
    Create data and model specifications files from a data frame of age
    readings that can be passed to
    [`run()`](http://pfmc-assessments.github.io/AgeingError/reference/run.md)
    or
    [`load_data()`](http://pfmc-assessments.github.io/AgeingError/reference/load_data.md)
    and
...
- Cleared out some, but not all, of the files associated with the ADMB
  version
...
- Added
  [`run()`](http://pfmc-assessments.github.io/AgeingError/reference/run.md)
  as a wrapper for the code provided by
  [@puntae](https://github.com/puntae)
...
The version used for the 2013 assessment cycle. Though the repository
was not tagged until 2015. Melissa Haltuch noted that in 2014 the code
was updated and a summary of those updates are provided below.

- tpl file
  - 2 new options for bias estimation.
    - 5.  Splines, which requires a new argument defining the knots.
    - 6.  Linear interpolation between knots, which requires a new
          argument that defines the knots.
  - Changes the calculation of the best true age because the previous
    way led to strange plots.
  - Improvement to effective sample size calculation.
- dat file
  - Read in one data set with missing entries, where there are no double
    reads. In the 2011-based code it allowed for reading in of multiple
    data sets.
  - Increased estimation speed because of how missing values are
    treated.
- R files
  ...
  - Plot standardized outputs from the output file. These figures could
    still use some development.
  - Ability to step through multiple runs and catalog the output using
    the new
