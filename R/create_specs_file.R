...
#' specs_file <- create_specs_file(dir = tempdir(), data = data, file_name = "test.spc")
#'
create_specs_file <- function(
  ...
  if (is.null(biasopt)) {
    ...
    cli::cli_alert_info("'biasopt' not specified; settings all readers to unbiased")
  }
  if (is.null(sigopt)) {
    sigopt <- c(1, rep(-1, nreaders - 1))
    cli::cli_alert_info("'sigopt' not specified; settings all readers to share a constant CV parameter")
  }
  if (any(sigopt %in% 5:6) && is.null(knotages)) {
    cli::cli_alert_abort("'knotages' must be specified when 'sigopt' includes 5 or 6")
  }

  # write file
  cli::cli_alert_info("Writing specifications file to {file.path(dir, file_name)}")
