...
`AgeingError` package that contains both the earlier AD Model Builder and
more recent TMB versions of the software. The TMB version is now recommended
and the ADMB version is available throught the git history.

There are two options for installing the TMB version, you can clone the source

https://github.com/pfmc-assessments/AgeingError.git

and build the package directly from the source,
...
remotes::install_github("pfmc-assessments/AgeingError@TMB")
```
...
```{r}
BG2022_dat <- AgeingError:::CreateData("data-raw/BG2022.dat", NDataSet=1, 
...
```{r}
BG2022_spc <- AgeingError:::CreateSpecs("data-raw/BG2022.spc", DataSpecs=BG2022_dat, 
                                       verbose=TRUE)
```
...
```{r}
BG2022final_dat <- AgeingError:::CreateData("data-raw/BG2022_trim_8_1.dat",
...
```{r}
BG2022final_spc <- AgeingError:::CreateSpecs("data-raw/BG2022_trim_8_1.spc",
...
```{r}
REB2022_dat <- AgeingError:::CreateData("data-raw/REB2022.dat", NDataSet=1, 
...
```{r}
REB2022_spc <- AgeingError:::CreateSpecs("data-raw/REB2022.spc",
...
<!-- set results="hide" -->
```{r, results="hide"}
REB2022_dat2 <- AgeingError:::CreateData("data-raw/REB2022_trim.dat", NDataSet=1, 
                  verbose=FALSE, EchoFile="REB2022_trimecho.out")
REB2022_spc2 <- AgeingError:::CreateSpecs("data-raw/REB2022_trim.spc",
...
```{r}
WHS2_dat <- AgeingError:::CreateData("data-raw/WHS2.dat", NDataSet=3, 
...
```{r}
WHS2_spc <- AgeingError:::CreateSpecs("data-raw/WHS2.spc",
...
```{r}
Sable_dat <- AgeingError:::CreateData("data-raw/Sable.dat", NDataSet=1, 
...
```{r}
Sable_spc <- AgeingError:::CreateSpecs("data-raw/Sable.spc",
