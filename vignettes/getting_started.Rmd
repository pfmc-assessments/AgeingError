...
The NOAA north west fisheries science center (NWFSC) GitHub site hosts the 
`nwfscAgeingError` package that contains both the earlier AD Model Builder and
more recent TMB versions of the software. At the time of writing this document
the TMB version was located in a branch of the git repository. 

There are two options for installing the TMB version, you can clone the source

https://github.com/pfmc-assessments/nwfscAgeingError.git

then navigate to the TMB branch and build the package directly from the source,
or, you can install the TMB branch directly from Github using the code below. Note
...
**Note it is important to install software from the TMB branch of the repository or you will get the older AD Model Builder version.**

The package can then be loaded. 

```{r}
## note the name is just AgeingError, not nwfscAgeingError
library(AgeingError)
```
...
R code to create the read comparison data for a data set is in the R package 
`SESSFdataproc` and examples are in the assessment repository `SESSFassessments`.
A version of this code is available from CSIRO. We usually create the
data file in Excel (using the summarised output from the R code) and then paste
into a text file.
...
* `-x` Assume that the relationship between expected age and true age is the same as that for reader x [note that the ''x'' must be lower than the number of the reader for which bias is being defined].
...
The C++ code (`agemat2.cpp`) also has conditional (if) statements for Bias 
options 4 and 5 but these appear to be identical to option 0 (unbiased).
...
*	`5` The standard deviation of random age-reading error, $\sigma_a$, is a Forsythe, Malcolm and Moler spline function of age. The number and location (ages) of the knots must be specified, with the number of parameters equal to the number of knots. Bounds and initial parameters need to be specified in log space.
...
The .spc file contains three sections, reader specification, knots for 
splines/piecewise linear functions, bias parameters and the sigma parameters.
...
```{r}
BG2022_dat <- AgeingError:::CreateData("data/BG2022.dat", NDataSet=1, 
                                       verbose=TRUE, EchoFile="BG2022echo.out")
```
When we set `verbose=TRUE` a summary of the loaded data is printedto the console.
...
```{r}
BG2022_spc <- AgeingError:::CreateSpecs("data/BG2022.spc", DataSpecs=BG2022_dat, 
                                       verbose=TRUE)
```
...
```{r}
BG2022final_dat <- AgeingError:::CreateData("data/BG2022_trim_8_1.dat",
...
```{r}
BG2022final_spc <- AgeingError:::CreateSpecs("data/BG2022_trim_8_1.spc",
...
```{r}
REB2022_dat <- AgeingError:::CreateData("data/REB2022.dat", NDataSet=1, 
...
```{r}
REB2022_spc <- AgeingError:::CreateSpecs("data/REB2022.spc",
...
<!-- set results="hide" -->
```{r, results="hide"}
REB2022_dat2 <- AgeingError:::CreateData("data/REB2022_trim.dat", NDataSet=1, 
                  verbose=FALSE, EchoFile="REB2022_trimecho.out")
REB2022_spc2 <- AgeingError:::CreateSpecs("data/REB2022_trim.spc",
...
```{r}
WHS2_dat <- AgeingError:::CreateData("data/WHS2.dat", NDataSet=3, 
...
```{r}
WHS2_spc <- AgeingError:::CreateSpecs("data/WHS2.spc",
...
```{r}
Sable_dat <- AgeingError:::CreateData("data/Sable.dat", NDataSet=1, 
...
```{r}
Sable_spc <- AgeingError:::CreateSpecs("data/Sable.spc",
