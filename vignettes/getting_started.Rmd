...
*Note: this vignette describes the AgeingError package as a whole including all 
the user options for data and specifications. See the separate vignette 
[R-based workflow for the AgeingError package](helper_functions.html) which 
details the `write_files()` function which makes it easier to write the 
correctly-formatted input files, and `run()` which makes it easy to run the 
model and process the results.*
