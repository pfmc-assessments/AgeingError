...
Nreaders <- 4
ReadsMat <- cbind(NumberOfReads=rep(25,5),
                  Reader1=c(1,1,0,0,1),
                  Reader1_DoubleReads=c(0,1,0,0,0),
                  Reader2=c(0,0,1,1,1),
                  Reader2_DoubleReads=c(0,0,0,1,0))
rownames(ReadsMat) <- c("Reader1_Only",
                        "Reader1_DoubleReads",
                        "Reader2_Only",
                        "Reader2_DoubleReads",
                        "Reader1_&_Reader2")
...
# Generate data
AgeReads <- SimulatorFn(Nreaders=Nreaders, M=0.2, SelexForm="Logistic",
                        SelexParams=c(5,0.2), BiasParams=c(1,1,1.1,1.1),
                        ErrorParams=c(0.2,0.2,0.2,0.2), ReadsMat=ReadsMat,
                        RecCv=0.6, RecAr1=0.8, Amax=100)
write.csv(AgeReads, file=file.path(DateFile,"Simulated_data_example.csv"))
...
for(RowI in 2:nrow(AgeReads)){
  ...
  for(PreviousRowJ in 1:nrow(AgeReads2)){
    if(all(AgeReads[RowI,1:Nreaders]==AgeReads2[PreviousRowJ,1:Nreaders+1])){
      DupRow <- PreviousRowJ
    }
  }
  if(is.na(DupRow)){
...
# 5 = Spline with estimated slope at beginning and end
#     (Number of params = 2 + number of knots)
...
RunFn(Data=AgeReads2, SigOpt=SigOpt, KnotAges=KnotAges, BiasOpt=BiasOpt,
